...
const submitContact = async (req, res) => {
  try {
    ...
    if (subject) {
       try {
          if (subject.includes("Studio")) departmentId = await Department.resolveId("Studio");
          else if (subject.includes("Stationery") || subject.includes("Papeterie")) departmentId = await Department.resolveId("Papeterie");
          else if (subject.includes("Flower")) departmentId = await Department.resolveId("Flower Gifts");
          else if (subject.includes("Fashion")) departmentId = await Department.resolveId("Classic Fashion");
       } catch(e) {}
    }
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
          ...
          <div style="padding: 30px; line-height: 1.6; color: #333;">
            <h2 style="color: #1B5E20; margin-top: 0;">New Inquiry Received</h2>
            <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
            <p><strong>Name:</strong> ${name}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Subject:</strong> ${subject}</p>
            <p><strong>Message:</strong></p>
            <div style="background: #f8fafc; padding: 16px; border-radius: 8px; border-left: 4px solid #1B5E20;">
              ${message}
            </div>
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
          ...
          <div style="padding: 36px 32px; color: #333; line-height: 1.7;">
            <h2 style="color: #1B5E20; margin: 0 0 12px; font-size: 20px;">Thank you, ${name}! 🙏</h2>
            ...
            <p style="margin: 4px 0; font-weight: 700; color: #1B5E20;">The DRAVANUA HUB Team</p>
          </div>
...
const getMessages = async (req, res) => {
  try {
    const whereClause = {};
    if (req.user && req.user.role !== 'super_admin' && req.user.departmentId && req.user.departmentId !== 'all') {
    ...
    const messages = await Message.findAll({ 
      where: whereClause,
      include: [
        { model: AdminUser, as: 'Sender', attributes: ['profilePicture', 'name'] }
      ],
...
const replyToMessage = async (req, res) => {
  try {
      ...
      html: `
        <div style="font-family: 'Inter', Arial, sans-serif; max-width: 650px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 25px rgba(0,0,0,0.05);">
          ...
          <div style="padding: 40px 35px; line-height: 1.7; color: #1e293b;">
            <h2 style="color: #1B5E20; margin-top: 0; font-size: 20px; font-weight: 800;">Hello ${message.senderName},</h2>
            ...
            <div style="background: #f8fafc; padding: 25px; border-radius: 12px; margin: 30px 0; border-left: 5px solid #1B5E20; color: #334155;">
            ...
            <div style="padding-top: 25px; border-top: 1px solid #f1f5f9;">
              <p style="margin: 0; font-weight: 800; color: #1B5E20;">Best Regards,</p>
    ...
    if (!result.success) {
      return res.status(500).json({ error: `Message resolved in DB, but email failed: ${result.error}` });
    }

    res.json({ success: true, data: message });
  } catch (error) {
    console.error("Failed to reply:", error);
    res.status(500).json({ error: "Failed to process reply. Ensure mailer is active." });
  }
};
