...
router.get("/due-today", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch due reminders." });
  }
});
  ...
  try {
    ...
    if (!title || !message || !reminderDate) {
      return res.status(400).json({
        success: false,
        message: "Title, Message, and Reminder Date are required.",
      });
    }
...
router.post("/send-email/:id", async (req, res) => {
  try {
      ...
      recipients = [
        {
          ...
          email: req.body.recipientEmail || reminder.recipient,
        },
      ];
    }
    ...
    if (recipients.length === 0) {
      return res.status(400).json({
        success: false,
        message: "No valid recipient found. Please specify a recipient email.",
      });
    }
    ...
    const htmlBody = `
      <div style="font-family: 'Segoe UI', Inter, sans-serif; max-width: 640px; margin: 0 auto; background: #f8fafc;">
        ...
        <div style="background: white; padding: 28px 32px;">
          <div style="background: #f8fafc; border-left: 4px solid #32FC05; padding: 18px 20px; border-radius: 0 12px 12px 0; margin-bottom: 24px;">
    ...
    if (successCount > 0) {
      ...
      res.json({
        success: true,
        message: `Reminder sent to ${successCount} recipient${successCount > 1 ? "s" : ""}.`,
      });
    } else {
      res.status(500).json({
        success: false,
        message: lastError || "Failed to send reminder email.",
      });
    }
  } catch (error) {
